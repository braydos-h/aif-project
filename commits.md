...
## 2026-08-24 — Redo WebUI as Field Instrument
- Rebuilt `web/index.html` as an editorial instrument: sticky nav with health pill, 2-column hero (kicker + 5rem display type with lime `photo.` accent + spec card with 250–900 kg / ≤15 MB / PRIVATE stats + ticker), 1.08fr/0.92fr dashboard grid, and session history. Preserved every `id` (`dropzone`, `preview-wrap`, `estimate-button`, `health-dot`, `weight-kg`, etc.) so `web/app.js` and the Rust `include_bytes!` routes stay contract-compatible.
...
- **Follow-up (`d59259f`):** ported the three quick-wins WebUI guards into `backend/tests/server.rs` (additive, 32+/0-) after the Python removal deleted `tests/test_webui.py`; reverted an accidental whole-file `cargo fmt` sweep to keep the peer session's diff clean. `cargo test` green 4 consecutive runs (33 lib + 3 + 33 integration, incl. the 3 new guards).

## 2026-09-08 (session: zero-Python follow-ups)
- **Context:** peer session landed the Python removal (`73342e2`: deleted `aif/`, `app.py`, `gui.py`, `pyproject.toml`, `tests/`) plus web quick-wins and 3 ported WebUI guard tests (`d59259f`); my working tree held the docs/CI/launcher follow-ups.
...
## 2026-09-15 03:38 UTC (session: backend hardening — SSRF guard, concurrency cap, /metrics)
- **Context:** user picked the backend-hardening batch. `image_url` was fetched with no host checks and redirects followed (SSRF to loopback/private/cloud-metadata); the server spawned one thread per connection with no cap; no operator counters existed (`/health` had no uptime, no `/metrics`).
- **Change (backend only, no new deps; `web/` untouched):**
  - `backend/src/validate/fetch.rs`: new `check_image_url_allowed` SSRF guard (plain `http(s)` shape, no `@`/`?`/`#`/whitespace, ≤2 KiB; literal-IP public-routability check incl. IPv4-mapped IPv6; `localhost`/`.local`/`.internal`/`.lan`/`.home`/`.corp`/`.intranet`/metadata hostnames blocked; best-effort DNS resolution refused when it resolves private); `fetch_url` runs the guard first, builds ureq with `redirects(0)` + 30 s timeout, and rejects non-2xx (redirects not followed). 6 new unit tests.
  - `backend/src/http/server.rs`: `MAX_CONCURRENT_CONNECTIONS = 64` with an RAII active-slot guard; over-limit connections get immediate `503 server_busy` JSON instead of a new thread. `response.rs`: new `CODE_SERVER_BUSY` + `503 Service Unavailable` text.
  - `backend/src/http/mod.rs` + `handlers.rs`: new `ServerMetrics` (start/total/active/rejected, `ServerState::new` constructor); `dispatch` counts requests; `/health` gains `uptime_secs` + `active_connections`; new `GET /metrics` (version/backend/model/uptime/total/active/rejected/cache size, no secrets); `/info` endpoints list gains `GET /metrics`. `main.rs` uses `ServerState::new`.
  - `backend/tests/server.rs`: `over_limit_image_url_rejected` now accepts with a deadline (loopback is SSRF-blocked before any fetch, so no connection may arrive); new `private_image_url_is_blocked_before_fetch` (127/10/192.168/metadata/localhost → 400 `invalid_image` containing "blocked"), `metrics_endpoint_reports_counters`, `health_reports_uptime_and_connections`.

...
A Tkinter app (``CowWeightApp``) that wraps ``CowWeightEstimator``
...
- Keep Tk and backend logic separate: reuse ``CowWeightEstimator`` from
  ``aif.estimator`` instead of inlining estimation code here.
"""
...
class CowWeightApp:
    ...
    Note:
        ...
        estimation itself is delegated to ``CowWeightEstimator``. A fresh
        estimator is built per request from the UI-selected backend/model/
        URL.
    """
